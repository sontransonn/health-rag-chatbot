...
if __name__ == "__main__":
    input_path = "datasets/symptoms_diseases/raw.json"
    output_dir = "datasets/symptoms_diseases/processed/"
    ...
    for idx, disease in enumerate(raw_data):
        information = disease.get("information", "")
        if not information.strip():
            continue

        metadata = {
            "id": normalize_field(disease.get("id", "")),
            "name": normalize_field(disease.get("name", "")),
            "source": normalize_field(disease.get("source", ""))
        }

        chunks = chunk_text(clean_text(information), max_words=200, overlap=1)
        for i, chunk in enumerate(chunks, start=1):
            all_chunks.append({
                "chunk_id": f"{disease.get("id", f"{idx}")}_{i}",
                "content": chunk,
                "metadata": metadata
            })
